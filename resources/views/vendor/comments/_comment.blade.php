...
<div id="comment-{{ $comment->getKey() }}" class="media border-bottom mt-3">
    
    @if($user->avatar && File::exists($user->avatar))
        <img class="mr-3 rounded-circle" src="{{ asset($user->avatar) }}" height="40"
             alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">
    @else
        <img class="mr-3 rounded-circle" src="{{ asset('assets/images/users/avatar.jpg') }}" height="40"
             alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar Avatar">
    @endif
    <div class="media-body">
        <h5 class="mt-0 mb-2">{{ $comment->commenter->name ?? $comment->guest_name }} <small class="text-muted">- {{ $comment->created_at->diffForHumans() }}</small></h5>
        <div style="white-space: pre-wrap;">{!! $markdown->line($comment->comment) !!}</div>

        <div>
            @can('reply-to-comment', $comment)
                <button data-toggle="modal" data-target="#reply-modal-{{ $comment->getKey() }}" class="btn btn-sm btn-out text-uppercase"><i class="fa fa-pencil-alt"></i></button>
            @endcan
         
            @can('delete-comment', $comment)
                <a href="{{ route('comments.destroy', $comment->getKey()) }}" onclick="event.preventDefault();document.getElementById('comment-delete-form-{{ $comment->getKey() }}').submit();" class="btn btn-sm btn-link text-danger text-uppercase"><i class="fa fa-trash"></i></a>
